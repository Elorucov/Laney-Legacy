...
namespace Elorucov.Laney.Controls
{
    public sealed partial class BotKeyboardControl : UserControl
    {
        ...
        private async void DrawKeyboard(BotKeyboard keyboard)
        {
            ButtonsArea.Children.Clear();
            ButtonsArea.Padding = new Thickness(0);
            Root.Visibility = keyboard == null ? Visibility.Collapsed : Visibility.Visible;
            ...
            foreach (var row in keyboard.Buttons)
            {
                ...
                for (int i = 0; i < row.Count; i++)
                {
                    ...
                    Button btn = MessageKeyboardHelper.GenerateButton(OwnerMessageId, row[i], this, ButtonClicked);
