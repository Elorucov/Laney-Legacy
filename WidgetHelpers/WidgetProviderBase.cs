...
namespace WidgetHelpers {
    public class WidgetProviderBase : IWidgetProvider {
        private record WidgetCreationInfo (string WidgetName, Func<WidgetContext, string, IWidget> Factory);

        public static T CreateWidget<T>(WidgetContext widgetContext, string state) where T : WidgetBase, new() {
            Debug.WriteLine($"WidgetProvider > CreateWidget — Id: {widgetContext.Id}; DefinitionId: {widgetContext.DefinitionId}; Size: {widgetContext.Size}; IsActive: {widgetContext.IsActive}");
        ...
        protected static void RegisterWidget<TWidget>(string widgetName)
            ...
            Debug.WriteLine($"WidgetProvider > RegisterWidget — Name: {widgetName}");
        ...
        void RecoverRunningWidgets() {
            try {
                ...
                foreach (var widgetInfo in infos) {
