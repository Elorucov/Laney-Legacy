...
namespace Elorucov.Laney.ViewModel {
    public class AudioPlayerViewModel : BaseViewModel {
        ...
        private AudioPlayerViewModel(List<Audio> songs, Audio currentSong, string name) {
            Log.Info($"APVM type=audio, count={songs.Count}, current={currentSong.Id}");
            ...
            new System.Action(async () => { await SwitchSongAsync(); })();

            PropertyChanged += async (a, b) => {
                if (b.PropertyName == nameof(CurrentSong)) await SwitchSongAsync();
        ...
        private AudioPlayerViewModel(List<Podcast> podcasts, Podcast currentPodcast, string name) {
            Log.Info($"APVM type=podcast, count={podcasts.Count}, current={currentPodcast.Id}");
            ...
            PropertyChanged += async (a, b) => {
                if (b.PropertyName == nameof(CurrentSong)) await SwitchSongAsync();
                if (b.PropertyName == nameof(RepeatOneSong)) AppParameters.AudioPlayerRepeat = RepeatOneSong;
                if (b.PropertyName == nameof(PlaybackState)) {
                    PlaybackStateChanged?.Invoke(this, PlaybackState);
                }
                ;
        ...
        private AudioPlayerViewModel(List<AudioMessage> messages, AudioMessage currentMessage, string ownerName) {
            Log.Info($"APVM type=voicemessage, count={messages.Count}, current={currentMessage.Id}");
        ...
        private void Initialize() {
            ...
            RepeatOneSong = Type != AudioType.VoiceMessage ? AppParameters.AudioPlayerRepeat : false;
            Log.Info($"APVM initialized. Repeat={RepeatOneSong}");
            ...
            RepeatCommand = new RelayCommand(o => {
                ...
                AppParameters.AudioPlayerRepeat = RepeatOneSong;
            });
            ShareCommand = new RelayCommand(async o => {
                await CoreApplication.MainView.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, async () => {
                    Main.GetCurrent().StartForwardingAttachments(new List<AttachmentBase> { CurrentSong.Attachment });
                    var id = ApplicationView.GetForCurrentView().Id;
                    await ApplicationViewSwitcher.TryShowAsStandaloneAsync(id, ViewSizePreference.Default);
                });
            });
        }

        private void Uninitialize() {
            Log.Info($"APVM initialized. Loop={RepeatOneSong}");
            Player.Dispose();
            Player = null;
        }

        private async Task SwitchSongAsync() {
            if (CurrentSong == null) return;

            Pause();
            Position = TimeSpan.FromMilliseconds(0);
            CurrentSongIndex = Songs.IndexOf(CurrentSong) + 1;
            Log.Info($"APVM changing audio to {CurrentSongIndex}");

            if (CurrentSong.Source.AbsolutePath.EndsWith(".m3u8")) {
                // ВК возвращает ссылку с параметром ?siren=1, который ломает воспроизведение. Ниже убираем этот параметр (и любые другие в будущем)
                var result = await AdaptiveMediaSource.CreateFromUriAsync(new Uri("https://" + CurrentSong.Source.DnsSafeHost + CurrentSong.Source.AbsolutePath));
                Log.Info($"APVM adaptive media source creation result={result.Status}, url={CurrentSong.Source}");
