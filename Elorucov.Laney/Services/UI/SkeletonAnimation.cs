...
namespace Elorucov.Laney.Services.UI {
    public class SkeletonAnimation {
        ...
        public static async Task FillPanelAndStartAnimation(Panel container, DataTemplate template, Color color, int count, FrameworkElement overrided = null) {
            await FillPanelAndStartAnimation(container, new List<DataTemplate> { template }, color, count, overrided);
        }

        public static async Task FillPanelAndStartAnimation(Panel container, List<DataTemplate> templates, Color color, int count, FrameworkElement overrided = null) {
            ...
            await Start(overrided != null ? overrided : container, animatableElements, color);
        }

        static Dictionary<FrameworkElement, List<Shape>> NonReadyElements = new Dictionary<FrameworkElement, List<Shape>>();

        public static async Task Start(FrameworkElement container, List<Shape> animatableElements, Color color) {
            if (container == null) throw new ArgumentNullException(nameof(container));
            ...
            if (container.ActualWidth == 0) {
                if (NonReadyElements.ContainsKey(container)) {
                    NonReadyElements[container].AddRange(animatableElements);
                } else {
                    NonReadyElements.Add(container, animatableElements);
                }
                container.Tag = color;
            ...
            foreach (Shape shape in animatableElements) {
                ...
                if (isGradientSupported) {
                    ...
                    gradient.ColorStops.Add(_compositor.CreateColorGradientStop(0.2f, color));
                    gradient.ColorStops.Add(_compositor.CreateColorGradientStop(0.3f, color));
                ...
                // Возможно, код ниже и был причиной краша при выходе из акка.
        ...
        private static void Container_SizeChanged(object sender, SizeChangedEventArgs e) {
            ...
            List<Shape> animatableElements = null;
            if (!NonReadyElements.TryGetValue(container, out animatableElements)) return;
            Color color = (Color)container.Tag;
            NonReadyElements.Remove(container);
            new Action(async () => { await Start(container, animatableElements, color); })();
        }
