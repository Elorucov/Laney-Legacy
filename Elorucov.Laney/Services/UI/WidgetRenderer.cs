...
namespace Elorucov.Laney.Services.UI {
    public class WidgetRenderer {
        public static FrameworkElement Render(Widget widget, Action<Uri> actionRequested) {
            try {
                switch (widget.Item.Type) {
                    case "universal_informer":
                        return BuildInformer(widget.Item.Payload, actionRequested);
                    case "universal_counter":
                        return BuildCounter(widget.Item.Payload, actionRequested);
                    case "universal_table":
                        return BuildTable(widget.Item.Payload, actionRequested);
                    case "universal_internal":
                        return BuildInternal(widget.Item.Payload, actionRequested);
                    default:
                        return new DefaultAttachmentControl {
                            Margin = new Thickness(12),
                            IconTemplate = (DataTemplate)Application.Current.Resources["Icon24Error"],
                            Title = Locale.Get("unknown_attachment"),
                            Description = $"Type: {widget.Item.Type}"
                        };
                }
            } catch {
                return new DefaultAttachmentControl {
                    ...
                    Description = $"Widget rendering error"
                };
            }
        }

        private static FrameworkElement BuildInformer(JObject payload, Action<Uri> actionRequested) {
            ...
            if (payload.ContainsKey("rows")) {
                foreach (var row in payload["rows"]) {
                    if (row is JObject rowobj) container.Children.Add(BuildWidgetInformerRow(rowobj, rootStyle, actionRequested));
                }
            }

            if (payload.ContainsKey("footer")) {
                FrameworkElement footer = BuildFooter((JObject)payload["footer"], actionRequested);
                if (footer != null) {
            ...
            return WrapToActionButton(container, (JObject)payload["action"], actionRequested);
        }

        private static FrameworkElement BuildCounter(JObject payload, Action<Uri> actionRequested) {
            StackPanel container = new StackPanel();
            ...
            if (payload.ContainsKey("items")) {
                ...
                foreach (var item in payload["items"]) {
                    ...
                    var counterItem = BuildWidgetCounterItem((JObject)item, rootStyle, actionRequested);
        ...
        private static FrameworkElement BuildTable(JObject payload, Action<Uri> actionRequested) {
            ...
            foreach (var rowItem in items) {
                ...
                foreach (var columnItem in rowItem) {
                    ...
                    cell = WrapToActionButton(cell, (JObject)columnItem["action"], actionRequested);
        ...
        private static FrameworkElement BuildInternal(JObject payload, Action<Uri> actionRequested) {
            var rootStyle = payload["root_style"];
            var headerIcon = payload["header_icon"];
            ...
            BitmapImage iconimg = new BitmapImage();
            iconimg.SetUriSourceAsync(new Uri(iconUrl));
            ...
            Border icon = new Border {
                ...
                Background = new ImageBrush() { ImageSource = iconimg, Stretch = Stretch.UniformToFill },
            };
            ...
            return WrapToActionButton(grid, (JObject)payload["action"], actionRequested);
        }
                        ...
                        <Border.Background>
                            <ImageBrush>
                                <ImageBrush.ImageSource>
                                    <BitmapImage Name=""icon"" DecodePixelType=""Logical"" DecodePixelWidth=""24"" DecodePixelHeight=""24""/>
                                </ImageBrush.ImageSource>
                            </ImageBrush>
                        </Border.Background>
                    </Border>
...
";
            ...
            BitmapImage iconimg = header.FindName("icon") as BitmapImage;
            iconimg.SetUriSourceAsync(new Uri(iconUrl));

            return header;
        }

        private static FrameworkElement BuildFooter(JObject footer, Action<Uri> actionRequested) {
            ...
            if (footer["type"].Value<string>() == "accent_button") {
                ...
                return WrapToActionButton(border, (JObject)payload["action"], actionRequested);
            } else {
                return null;
            }
        }
        ...
        private static FrameworkElement BuildWidgetInformerRow(JObject row, JToken rootStyle, Action<Uri> actionRequested) {
            ...
            return WrapToActionButton(grid, (JObject)row["action"], actionRequested);
        }
        ...
        private static FrameworkElement BuildWidgetInformerSideElement(JToken element, JToken style) {
            ...
            switch (type) {
                ...
                case "image":
                    ...
                    img.SetUriSourceAsync(new Uri(imageUrl));
        ...
        private static FrameworkElement BuildWidgetCounterItem(JObject item, JToken style, Action<Uri> actionRequested) {
            ...
            return WrapToActionButton(panel, (JObject)item["action"], actionRequested);
        }
        ...
        private static FrameworkElement WrapToActionButton(FrameworkElement element, JObject action, Action<Uri> actionRequested) {
            if (action != null && action["type"].Value<string>() == "open_url") {
                ...
                button.Click += (a, b) => {
                    ...
                    actionRequested?.Invoke(new Uri(url));
                };
                return button;
            } else {
                return element;
